...
@RequestMapping(path = "api/v1/smu/village/account/disable-village-account")
@Tag(name = "villageAccount",description = "Data REST API for village account resource")
public class VillageAccountDisableController {
    private final VillageAccountEventHandler eventHandler;

    @Operation(summary = "Disable a village account by code")
    @PostMapping("/a-village-account-by-code")
    public Mono<ResponseEntity<MessageResponse>> disable(@Valid @RequestBody VillageAccountEnableCommand command) {
        try {
